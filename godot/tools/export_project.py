...
LOCAL_TEMPLATES = PROJECT.parent / "test-artifacts/godot/export-templates"
...
def export(
    ...
    with tempfile.TemporaryDirectory(prefix="nathaniel-godot-export-") as directory:
        ...
        shutil.copytree(
            ...
            ignore=shutil.ignore_patterns(
                ".godot", "exports", "build", "__pycache__", ".git"
            ),
        )
...
def main() -> None:
    ...
    output = args.output or PROJECT / "exports" / args.platform.lower() / (
        "NathanielGodot.app" if args.platform == "macOS" else "NathanielGodot.zip"
    )
    try:
        status = export(
            args.platform,
