import Foundation

// MARK: - Threat Constants

/// Configuration for threat system (delegates to GameBalance.Threat)
enum ThreatConfig {
    /// Base threat generated per point of damage dealt
    static var damageThreatMultiplier: Float { GameBalance.Threat.damageThreatMultiplier }

    /// Threat generated when first entering visible range
    static var initialAggroThreat: Float { GameBalance.Threat.initialAggroThreat }

    /// Proximity threat per second when in visible range
    static var proximityThreatPerSecond: Float { GameBalance.Threat.proximityThreatPerSecond }

    /// Percentage of threat that decays per second (0.0-1.0)
    /// At 0.10 (10%), threat halves roughly every 7 seconds
    /// At 0.15 (15%), threat halves roughly every 4.5 seconds
    static var threatDecayRatePerSecond: Float { GameBalance.Threat.threatDecayRatePerSecond }

    /// Minimum threat to consider a valid target
    static var minimumThreatThreshold: Float { GameBalance.Threat.minimumThreatThreshold }

    // MARK: - Tower-Specific Threat

    /// Multiplier for threat generated by tower damage (lower than players)
    /// At 0.7, enemies prefer attacking players over towers
    static var towerDamageThreatMultiplier: Float { GameBalance.Threat.towerDamageThreatMultiplier }

    /// Proximity threat per second for towers (half of players)
    static var towerProximityThreatPerSecond: Float { GameBalance.Threat.towerProximityThreatPerSecond }
}

// MARK: - Threat Table

/// Tracks threat values for potential targets
/// Used by enemies to determine which player to attack
struct ThreatTable {
    /// Threat entry for a single target
    private struct ThreatEntry {
        weak var target: Damageable?
        var threat: Float
    }

    /// Threat entries by target (using ObjectIdentifier as key)
    private var entries: [ObjectIdentifier: ThreatEntry] = [:]

    // MARK: - Threat Modification

    /// Add threat for a target
    /// - Parameters:
    ///   - target: The entity generating threat (Character or Structure)
    ///   - amount: Amount of threat to add
    mutating func addThreat(for target: Damageable, amount: Float) {
        let id = ObjectIdentifier(target)
        if var entry = entries[id] {
            entry.threat += amount
            self.entries[id] = entry
        } else {
            self.entries[id] = ThreatEntry(target: target, threat: amount)
        }
    }

    /// Get current threat value for a target
    /// - Parameter target: The entity to check
    /// - Returns: Current threat value (0 if not tracked)
    func getThreat(for target: Damageable) -> Float {
        let id = ObjectIdentifier(target)
        return self.entries[id]?.threat ?? 0
    }

    /// Apply threat decay over time (percentage-based)
    /// - Parameter deltaTime: Time since last update
    mutating func decayThreat(deltaTime: TimeInterval) {
        // Calculate decay multiplier for this frame
        // Using (1 - rate)^time for smooth exponential decay
        let decayMultiplier = pow(1.0 - ThreatConfig.threatDecayRatePerSecond, Float(deltaTime))

        var idsToRemove: [ObjectIdentifier] = []

        for (id, var entry) in entries {
            // Skip dead or deallocated targets
            guard let target = entry.target, target.isAlive else {
                idsToRemove.append(id)
                continue
            }

            entry.threat *= decayMultiplier

            // Remove if below threshold
            if entry.threat < ThreatConfig.minimumThreatThreshold {
                idsToRemove.append(id)
            } else {
                self.entries[id] = entry
            }
        }

        // Clean up
        for id in idsToRemove {
            self.entries.removeValue(forKey: id)
        }
    }

    /// Clear all threat (for reset)
    mutating func clear() {
        self.entries.removeAll()
    }

    // MARK: - Target Selection

    /// Get the target with highest threat
    /// - Returns: The entity with highest threat, or nil if no valid targets
    func getHighestThreatTarget() -> Damageable? {
        var highestThreat: Float = 0
        var highestTarget: Damageable?

        for entry in self.entries.values {
            guard let target = entry.target, target.isAlive else { continue }

            if entry.threat > highestThreat {
                highestThreat = entry.threat
                highestTarget = target
            }
        }

        return highestTarget
    }

    /// Get all valid targets sorted by threat (highest first)
    /// - Returns: Array of (target, threat) pairs sorted by threat
    func getSortedTargets() -> [(target: Damageable, threat: Float)] {
        var result: [(Damageable, Float)] = []

        for entry in self.entries.values {
            guard let target = entry.target, target.isAlive else { continue }
            result.append((target, entry.threat))
        }

        return result.sorted { $0.1 > $1.1 }
    }

    /// Check if we have any valid targets
    var hasTargets: Bool {
        self.entries.values.contains { entry in
            guard let target = entry.target else { return false }
            return target.isAlive
        }
    }

    /// Number of tracked targets
    var count: Int {
        self.entries.values.count(where: { $0.target?.isAlive == true })
    }
}
