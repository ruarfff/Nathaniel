...
//  Nathaniel Shared
//
//  Utility for programmatically injecting touch/click events into SpriteKit scenes.
...
extension SKScene {
    ...
    func touchesBegan(at point: CGPoint) {
        // Create a simple struct to pass the point to existing handlers
        // We use the scene's handleTap method if available
        if let gameScene = self as? GameScene {
            gameScene.handleTap(at: point)
